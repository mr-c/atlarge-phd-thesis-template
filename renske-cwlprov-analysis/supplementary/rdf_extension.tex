...
\label{sec:renske:dev_recommendatons}

\input{renske-cwlprov-analysis/rdf_extension/rdf_extension.tex}

\input{renske-cwlprov-analysis/rdf_extension/rdf_extension_actions.tex}

In Section \emph{\nameref{sec:renske:cwlprov_evaluation}}, we analyzed CWLProv for the representation of the provenance taxonomy we defined in \emph{\nameref{sec:renske:user_req}}. Based on this analysis, we concluded that not all provenance components were sufficiently represented in a structured format. Specifically, we found that although certain metadata was part of \emph{primary-job.json} and \emph{packed.cwl}, these annotations were not represented in RDF format.
...
In Section \emph{\nameref{sec:renske:ext_reqs}}, we first describe the requirements and principles which we used in this design. Subsequently, in Section \emph{\nameref{sec:renske:ext_overview}}, we give a high-level overview of the design. We then give recommendations for specific terms and vocabulary which can be used for the metadata fields \emph{doc}, \emph{label}, \emph{intent}, and \emph{format} which are part of v1.2 of the CWL Standards (Section \emph{\nameref{sec:renske:ext_details}}). We describe how we have partially realized this design in \emph{cwltool} (Section \emph{\nameref{sec:renske:realization}}). Finally, we provide a list of SPARQL queries in Section \emph{\nameref{renske:sup:sparql}}.

\subsection{Requirements and principles}
\label{sec:renske:ext_reqs}
For this design, we reuse principles \ref{renske:pr:bioschemas} and \ref{renske:pr:current_standards}. In addition, the RDF extension should adhere to four requirements.

\begin{enumerate}[label=\textbf{PR\arabic*}]
    \item \label{renske:pr:metadata_fields} Represent the annotations that are supported in CWL workflows according to the CWL standards v1.2, summarized in Table \ref{tab:renske:metadata_fields}.
    \item \label{renske:pr:input} Represent the annotations for \emph{Files} and \emph{Directories} proposed in Section \emph{\nameref{sec:renske:_annot_cwl_now}}.
    \item \label{renske:pr:execution} Represent the annotations for a collection of input values proposed in Section \emph{\nameref{sec:renske:annot_cwl_now}}.
    \item \label{renske:pr:actions} Represent the annotations for Actions proposed in Section \emph{\nameref{sec:renske:annot_new_cwl}}.
\end{enumerate}

\subsection{High-level overview}
\label{sec:renske:ext_overview}
In this section, we present a high-level overview of the extended provenance graph and link it to the requirements defined in Section \emph{\nameref{sec:renske:ext_reqs}}.

\subsubsection{Representation of CWL metadata fields}

\input{renske-cwlprov-analysis/related_work/table_cwl_metadata.tex}

Figure \ref{fig:renske:cwlprov_graph_new} shows how we extended the provenance graph to represent all current metadata fields (\ref{renske:pr:metadata_fields}). \textbf{In the new graph, every workflow component in Table \ref{tab:renske:metadata_fields} is represented as a distinct entity, and interrelated with terms from the \emph{wfdesc} ontology (\ref{renske:pr:bioschemas}).} 
...
This structure depicted in Figure \ref{fig:renske:cwlprov_graph_new} also supports representation of the input data annotations described in \emph{\nameref{sec:renske:annot_cwl_now}} (\ref{renske:pr:input}), by transferring them to the data entities they describe (\textbf{G5}). 


\subsubsection{Representation of configuration settings annotations} Annotations describing a collection of parameters (\ref{renske:pr:execution}) are specific to the workflow execution. Therefore, we recommend that these annotations are transferred to the entity in the provenance graph describing the workflow execution (\textbf{G2}).

\subsubsection{Representation of input data history}
In the annotation scheme described in Section \emph{\nameref{sec:renske:annot_new_cwl}}, the history of (processed) input data is expressed through \emph{Actions}. Figure \ref{fig:renske:cwlprov_graph_actions} presents their representation in RDF (\ref{renske:pr:actions}). 
Actions (\textbf{A2}) are performed on objects, which are data entities not aggregated in the CWLProv RO (\textbf{A1}). Actions can have additional properties such as Tools (\textbf{A3}) or other annotations as summarized in Table \ref{tab:renske:action_schemaorg}. They are connected to input data entities (\textbf{G5}) via \emph{s:result}.

\subsection{Details of the design}
\label{sec:renske:ext_details}
...
When entities are described with nested annotations (e.g. \emph{s:citation}), we recommend to make this a separate entity in the graph instead of a nested annotation, in compliance with the PROV data model (\ref{renske:pr:bioschemas}).

\subsection{Partial realization in \emph{cwltool}}
\label{sec:renske:realization}
\todomichael{Explain how cwltool was updated}
Because of time constraints, we could only partially realize the provenance graph extension in \emph{cwltool}. At the time of writing, annotations directly associated with inputs of type \emph{File} and \emph{Directory}, as exemplified in \emph{\nameref{sec:renske:annot_fair}}, are propagated to the RDF provenance record. However, annotations of the collection of input parameter values (\emph{\nameref{sec:renske:annot_actions}}) or annotations under \emph{cwlprov:prov} (\emph{\nameref{sec:renske:annot_new_cwl}}), are currently not represented in RDF.

\subsection{Conceptual analysis of the design extension}
\label{sec:renske:ext_analysis}

\todorenske{Update once cwltool is updated}
